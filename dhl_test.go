...
func Test001(m *testing.T) {
	...
	currentTime := time.Now().Add(24 * time.Hour) // 加上 24 小时，即明天的此刻
...
func Test002(m *testing.T) {
	...
	loc, _ := time.LoadLocation("Asia/Shanghai")
	currentTime := time.Now().Add(24 * time.Hour) // 加上 24 小时，即明天的此刻
	formattedTime := currentTime.In(loc).Format("2006-01-02T15:04:05 GMT+08:00")
	accounts := []model.Account{
		{
			Number:   cfg.CustomerCode,
			TypeCode: "shipper",
	...
	customerDetails := model.CustomerDetails{
		ReceiverDetails: &model.PartyDetails{
			ContactInformation: &model.ContactInfo{
				FullName:    "Adam Spencer",
				Email:       "that@before.gb",
				Phone:       "+1123456789",
		...
		ShipperDetails: &model.PartyDetails{
			ContactInformation: &model.ContactInfo{
				FullName:    "张三",
				Email:       "zhangsan@example.com",
				Phone:       "+8613645678890",
	...
	postData := model.CreateShipmentData{
	...
	resp, err := DhlClient.Services.Base.ShipmentCreate(postData)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
	}
	fmt.Println("666", resp)
}
